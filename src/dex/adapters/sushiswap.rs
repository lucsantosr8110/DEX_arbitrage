// ============================================================
// src/dex/adapters/sushiswap.rs — v6.1.2 (FINAL FIXES)
// ============================================================
...
#[async_trait]
impl DexContract for SushiSwapDex {
    fn name(&self) -> String {
        DEX_NAME.into()
    }
    ...
    async fn get_price(&self, token_a: &Address, token_b: &Address) -> Result<Option<f64>> {
        ...
        // C1: dimensionar amount_in pelo notional configurado (igual ao multicall).
        let symbol_a = self
            .token_cache
    ...
    async fn swap(&self, token_in: Address, token_out: Address, amount_in: U256) -> Result<U256> {
        if self.config.execution.dry_run {
            info!("[{}] Dry-run swap {:?} -> {:?} (SushiSwap)", DEX_NAME, token_in, token_out);
            return Ok(amount_in);
        }
        ...
        ALCHEMY_RATE_LIMITER.acquire().await?;
        let pending = method.send().await?;// Assumindo que o método é síncrono ou `send` resolve para um `PendingTransaction`
